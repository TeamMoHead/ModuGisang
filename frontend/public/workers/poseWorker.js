...
importScripts('/pose/pose.js');

let pose;

onmessage = function (e) {
  console.log(':: Messages from Mission1: ', e);

  const { data } = e;
  switch (data.command) {
    case 'load':
      pose = new Pose('/pose/pose_landmark_lite.tflite');
      console.log('::WORKER LOAD pose model=====> ', pose);
      pose.setOptions({
        modelComplexity: 0.5,
        selfieMode: true,
    ...
    case 'send':
      console.log('::WORKER SEND to pose model=====> ', data.image);
      pose
